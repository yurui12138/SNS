...
class ArxivSearchRM(dspy.Retrieve):
    def __init__(
        self,
        k: int = 3,
        is_valid_source: Callable = None,
    ...
    def forward(
        ...
        for query in queries:
            try:
                ...
                if not terms:
                    continue
                q = urllib.parse.quote_plus(" AND ".join([f"all:{t}" for t in terms]))
                url = (
                    f"http://export.arxiv.org/api/query?search_query={q}&start=0&max_results={self.k}&sortBy=relevance"
                )
                ...
                for entry in root.findall("atom:entry", ns):
                    ...
                    if not title or not summary or not link:
                        continue
                    if self.is_valid_source(link) and link not in exclude_urls:
                        collected_results.append(
                            {
                                "url": link,
                                "title": title,
