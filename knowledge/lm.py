...
# Disable caching to avoid AttributeError: __annotations__ in current environment
...
class LM:
    ...
    def __call__(self, prompt=None, messages=None, **kwargs):
        ...
        outputs = [
            (c.get("message", {}).get("content") if isinstance(c, dict) else getattr(c, "message", None).content)
            if (isinstance(c, dict) and c.get("message") or hasattr(c, "message"))
            else (c.get("text") if isinstance(c, dict) else getattr(c, "text", ""))
            for c in choices
        ]

        kwargs = {k: v for k, v in kwargs.items() if not k.startswith("api_")}
        entry = dict(prompt=prompt, messages=messages, kwargs=kwargs, response=response_dict)
        entry = dict(**entry, outputs=outputs, usage=dict(response_dict.get("usage", {})))
        entry = dict(
            **entry, cost=(response_dict.get("_hidden_params", {}) or {}).get("response_cost")
        )
...
class LitellmModel(LM):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._debug_dir = kwargs.get("debug_responses_dir")
    ...
    def __call__(self, prompt=None, messages=None, **kwargs):
        ...
        try:
            debug_dir = self._debug_dir or kwargs.get("debug_responses_dir")
            if debug_dir:
                import os, time
                os.makedirs(debug_dir, exist_ok=True)
                ts = time.strftime("%Y%m%d-%H%M%S")
                fname = f"{ts}-{self.model.replace('/', '_')}-{str(kwargs.get('tag') or 'call')}.json"
                path = os.path.join(debug_dir, fname)
                with open(path, "w", encoding="utf-8") as f:
                    ujson.dump(entry, f, indent=2, ensure_ascii=False)
        except Exception:
            pass
...
def to_dict(response):
    ...
    try:
        if hasattr(response, "json"):
            data = response.json()
            return data if isinstance(data, dict) else ujson.loads(data)
    except Exception:
        pass
